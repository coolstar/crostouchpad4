...
} CYAPA_CONTEXT, *PCYAPA_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(CYAPA_CONTEXT, GetDeviceContext)
...
#define CyapaPrint(dbglevel, dbgcatagory, fmt, ...) {          \
    if (CyapaDebugLevel >= dbglevel &&                         \
        (CyapaDebugCatagories && dbgcatagory))                 \
		    {                                                           \
        DbgPrint(DRIVERNAME);                                   \
